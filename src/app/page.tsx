...
async function loadOverview() {
  ...
  const [deploy] = await sql<
    { sha: string; version: string; summary: string }[]
  >`
    SELECT sha, version, summary
    FROM deployments
    WHERE service_id = ${service.id} AND status = 'active'
...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!service ? (
        <div className="panel p-4 text-sm text-[var(--warn)]">
              ...
              <ul className="space-y-2">
                {incidents.map((i) => (
                  <li key={i.id}>
                    <a
                      href={`/incidents/${i.id}`}
                      className="flex items-center justify-between rounded-lg border border-[var(--line)] px-3 py-2 hover:border-[var(--accent)]"
                    >
                      <span>{i.title}</span>
                      <span className="badge bg-[var(--line)] text-[var(--warn)]">
                        {i.status}
                      </span>
                    </a>
                  </li>
